...
if "role" not in st.session_state:
    st.title("📈 주식시장 사이클 시뮬레이터 (Cohn et al., 2015 실험)")
    ...
    if role == "학생":
        ...
        if st.button("실험실 입장", type="primary"):
            if name:
                ...
                if not res.data:
                    # 은밀하게 Boom / Bust 그룹 무작위 배정 (50% 확률)
...
if st.session_state.role == "student":
    ...
    if phase == "대기":
        ...
        if st.button("💼 최종 의사결정 제출", type="primary"):
            ...
            st.success("의사결정이 제출되었습니다! 새로고침 버튼을 눌러 상태를 업데이트하세요.")
            st.rerun()
